...
* Programming knowledge, ideally C or C-like languages like C++, along with the following programming concepts:
  * Pointers
  * Structs
  * Bitwise operations
...
- [Assembly primer](#assembly-primer)
  ...
  - [Instructions](#instructions)
    ...
    - [Load/Store main memory](#loadstore-main-memory)
    - [Branches](#branches)
...
>Behind this damage calculator is a complicated math formula, discovered through reverse engineering.
...
* A ROM of _Pokémon Mystery Dungeon: Red Rescue Team_ - the game we'll reverse engineer. Make sure to use the USA/NTSC version of the game; other versions like the EU/PAL version have differences that change the locations of data and functions in memory. I will not link to a download here, but it should be fairly easy to find.

### Setting up Ghidra
>![](/assets/img/reverse-engineering/ghidra.png)<br>
>Analyzing a function in Ghidra.

Ghidra is a free open-source reverse engineering tool developed by the U.S.'s National Security Agency (NSA). Ghidra can analyze binary data such as a game ROM to output assembly code and even decompiled C code, which we can read to reverse engineer a game's code. This kind of analysis occurs without the game running; this is known as __static code analysis__ (as opposed to __dynamic code analysis__ when debugging the game while it is running).

>Note: Ghidra is an ideal code analysis and disassembly tool for hobbyists, costing nothing for a fully featured application. There are paid alternatives to Ghidra; the industry standard for professional reverse engineering is [IDA](https://hex-rays.com/ida-pro/). However, IDA costs thousands of dollars, too costly for many hobbyists.

To install Ghidra, follow the instructions on [Ghidra's website](https://ghidra-sre.org/).

If you're on Mac, you'll need to give Ghidra's decompiler permission to run before setting up a project. Inside Ghidra's folder, go to `Ghidra/Features/Decompiler/os/mac_x86_64`, _right-click_ Open the `decompile` executable, and confirm the Open when macOS warns about not being able to verify the developer.
...
6. You'll need to select an assembly language for Ghidra to use for analyzing the binary. The GBA uses the ARM v4t little endian (__ARM:LE:32:v4t__) language, which you can find by searching for "v4t" in the language select, then selecting the option with "little" under the _Endian_ column.
    >![](/assets/img/reverse-engineering/armv4t.png)<br>
    >Selecting the ARMv4T language
7. Once you've selected the language, click _OK_ to make Ghidra inspect the binary.
...
10. Ghidra will offer to analyze the binary. Click _OK_.
11. Use the default analysis settings and click _Analyze_.
12. Ghidra will take some time to analyze the binary. It will attempt to find which parts of the code are functions, and then disassemble/decompile them.
...
How would you find that you should use the ARM:LE:32:v4t language if I didn't tell you? First search for which CPU the GBA uses; you'll find that it uses the ARM7TDMI (e.g., from [Wikipedia](https://en.wikipedia.org/wiki/Game_Boy_Advance)). Knowing the CPU, you can look for which language/architecture the CPU uses; [ARM's documentation](https://developer.arm.com/documentation/ddi0210/c/Introduction/Architecture) states that the ARM7TDMI uses the ARMv4T architecture.
...
In addition to Ghidra, you'll need to set up a GBA emulator and progress the game to the point where normal gameplay begins.
...
9. Once you enter Tiny Woods, you are in the main gameplay of Red Rescue Team, and are set up to reverse engineer it.
>![](/assets/img/reverse-engineering/mgba-setup.png)<br>
>Red Rescue Team after setting it up

With both Ghidra and the game set up, the next step is to open it up and read the code. You'll need to know how to read assembly language  for this, which the next section will introduce.

## Assembly primer
An [assembly language](https://en.wikipedia.org/wiki/Assembly_language) (ASM for short) is a type of low-level programming language that closely matches the raw binary instructions (machine code) read and executed by the computer. Compared to a higher-level language like C, assembly uses fewer abstractions; for instance, a function call is a single-line statement in C, but this operation corresponds to several lines of assembly code. We'll go into specific examples of this later on.

>This tutorial presents a brief introduction of assembly to get you started. For brevity, I'll be skipping some of the finer or lesser-used details. If you're interested in a more thorough GBA assembly reference, check out Tonc's [Whirlwind Tour of ARM Assembly](https://www.coranac.com/tonc/text/asm.htm#sec-arm). Aside from the assembly guide, Tonc also has detailed documentation on all the inner workings of the GBA console.

Different CPUs may use different assembly languages depending on which operations the CPU supports (the "instruction set" of the CPU). While all CPUs support the minimum set of instructions required for a computer to function, extra instructions act as shortcuts that allow a program to compile to fewer lines of assembly, resulting in faster program instruction in exchange for more complicated CPU hardware to support the additional instructions, and possibly each individual instruction taking more space to store. This tutorial will use the __THUMB__ instruction set, as this is the main instruction set used by Red Rescue Team's code. Future uses of the word "assembly" in this tutorial will be shorthand for "THUMB instruction set assembly language"; this is a common shorthand in the context of a specific game.
...
movs r0,#0x0
```
This statement assigns the number 0 to `r0`. `r0` is one of the CPU's registers. What are registers? Let's talk about that first.

### Registers
A __register__ is a quickly accessible location in the CPU hardware that stores a value. CPUs have a handful of registers available to them; the GBA's CPU has 16 registers, and each can store up to 32 bits (8 bytes) of data for a total of 512 bits (64 bytes) of data. Registers are named with the format `rX`, where `X` is the 0-indexed register number. In the GBA's case, its registers go from `r0` to `r15`. When the CPU needs to store data for later, the registers are the fastest place to store and retrieve the data. Every assembly operation interacts with the registers in some way.

Register capacity is limited, so if a program needs to store more data than the registers allow, the data goes to __main memory__, also known as __RAM__ (random-access memory) or simply __memory__. Storing and accessing memory is slower than using registers, but memory has a much larger capacity. The GBA has 288 kilobytes of RAM for regular code operations to use; it also has additional reserved RAM space for special operations like I/O.

`r15` is a special register called the __program counter__ (__PC__ for short). This register holds the address to the assembly instruction that the computer will execute next, and is incremented by 2 automatically after an instruction is executed (each instruction is 2 bytes long). This causes assembly code to be executed by the CPU line-by-line. It is possible to set the PC's value, which will cause the CPU to jump to the new address and begin executing code there; this is used for constructs like conditional statements, loops, and function calls.

Some of the other registers also have special names and purposes. We will get into those later.

### ROM
The ROM (read-only memory) file used to load the game is effectively an array of bytes. These bytes encode assets like sprites and audio, data values used by the game's logic (e.g., the amount that a certain healing item will heal the player by), and assembly code used to run the game.

When talking about data in the ROM, it is typical to refer to a piece of data according to its 0-indexed __offset__ (__address__) from the start of the ROM. For example, the first byte in the ROM file is at address 0x0, and the 5th byte in the ROM file is at address 0x4. Addresses are almost always written in hexadecimal; the "0x" indicates that a value is expressed in hexadecimal format. Hexadecimal is also commonly used for other values in reverse engineering contexts.

When the GBA loads the ROM, the ROM's data is loaded into a special area of memory starting at address 0x8000000. This means that in-game, values in the ROM are accessed using their offset plus 0x8000000. For example, the data at address 0x4 in the ROM is accessed in-game using offset 0x8000004.
...
Each instruction is encoded in either 2 or 4 bytes of data. Reverse engineering tools can read these bytes and convert them to a more human-readable form, which is usually how assembly code is expressed. This tutorial will use the human-readable form in the vast majority of cases.
...
> The extra 's' in the mnemonic `movs` is used for certain assembly languages to distinguish between different assignment operators that set flags or increment registers automatically to speed up common operations like string assignment. There is no such distinction in THUMB assembly, so `movs` and `mov` are used interchangeably. This 's' also appears in other mnemonics.
...
>You may notice the lack of a division instruction. Division with arbitrary numbers is more complicated than the operations listed above, so it is implemented as a function rather than a single instruction. This means division is much slower than other operations. Note that the right shift operator can divide numbers by powers of 2 to achieve certain divisions in a single instruction.

#### Load/Store main memory
...
str r0,[r1,#0x4]
```
* The mnemonic "str" can be read as "store register".
...
ldr r0,[r1,#0x4]
```
"ldr" can be read as "load register". This command will load from the address stored in `r1` plus the immediate offset of 4, storing the loaded value in `r0`. Like `str`, there are instructions to load halfwords (`ldrh`) and single bytes (`ldrb`).
...
b LAB_00090fde
```
`LAB_00090fde` is known as a __label__. The label represents a certain instruction in memory; in this case, the label refers to the instruction at address 0x90fde in the ROM.

Taking the previous example, if the branch instruction above is at address 0x8000002:
1. Execute the instruction at address 0x8000000.
2. Increment the program counter to 0x8000002.
3. Execute instruction `b LAB_00090fde` at 0x8000002.
...
beq LAB_00090f14
```
With this set of instructions, if the value of r0 is equal to 1, then the program to branch to `LAB_00090f14`. If r0 is not equal to 1, then the program skips the branch and moves to the next instruction instead.
...
A higher-level language like C uses conditional keywords like `if`/`else if`/`else` and looping keywords like `while`/`do while`/`for`. These constructs translate to conditional branch statements when compiled to assembly.
...
bl FUN_000450f8
```
`bl` is a special branch instruction used for functions. In this case, the program will jump to the first instruction of the function `FUN_000450f8`. Before branching, the current value of the program counter is saved to the __link register__ (`lr`), which corresponds to `r14` for the GBA's CPU. The link register's value will be retrieved at the end of the function to return the program to the place where the function was called.
...
By default, Ghidra names functions according to the memory address where they start at. This function starts at memory address 0x450f8, so the function is named `FUN_000450f8`.
...
In the code below, a value is assigned to `r0` to pass as an argument into `FUN_000450f8`.
```
...
adds r0,r4,#0x0
bl FUN_000450f8
...
#### Return values
If a function needs to return a value, the return value will be stored in `r0` just before the function returns. The caller function can then take the return value out of `r0` and use it as needed.

In function `FUN_00083654`, the function body assigns a value to `r0` before returning with `bx`. 
```
FUN_000450f8
...
#### Call stack
When a function is called, the caller is likely already using the registers to store values. There are only a handful of registers, and the function may also need those registers to do its work. Before the function can use the registers, it needs to save the existing values of registers it plans to use. When the function is finished, it must restore the saved values back to the registers so the caller doesn't lose its current state when it resumes execution.

Registers `r0`-`r3` are designated as __scratch registers__, which do not need to be saved by a function that uses them. Register `r4`-`r11` are __preserved registers__ whose values need to be saved and restored by the function. If the function calls other functions, this will overwrite the value of `lr`, so `lr` must also be saved, otherwise the function will lose the return address and be unable to return to the caller.
...
The stack starts at address 0x3007FFF and goes down to 0x3000000. The address at the top of the stack is tracked using `r13`, often called the __stack pointer__ (`sp`).
...
In Ghidra, stack local variables are assigned names such as `local_24` based on their position in the stack, rather than displaying the raw offset from `sp`.
...
add sp,#0x4
```
Like with stack local variables, the prologue subtracts from `sp` to allocate stack space for passing a function argument on the stack. When the function `FUN_00073b78` is called, `r0`-`r3` are used for four of the arguments, and the stack is used for the last argument (`str r2,[sp,#0x0]`). The epilogue adds to `sp` clean up the stack space.

Inside `FUN_00073b78`, the function access the stack argument via an offset from `sp`.
```
FUN_00073b78
...
```
ldr r0,[DAT_00073b70] // Load the address of a Position.
movs r1,#0x6
...
With both `ldmia` and `stmia`, the incremented register's addresses ends up at the address immediately after the last value loaded/stored. This makes it possible to chain `ldmia`/`stmia` to copy data of any size. A chain of these instructions may look like the following:
```
ldmia r1!,{ r3, r4, r5 }
stmia r2!,{ r3, r4, r5 }
ldmia r1!,{ r3, r4, r5 }
...
```
ldr r2,[DAT_00073b70] // Load pointer to start of array.
...
cmp r6,#0x5
ble LAB_00073ac0 // Go back to loop start if iteration is not finished.
```
...
### Switch statements
In assembly, switch statements are implemented using a __jump table__. An array of instruction addresses is stored in assembly, and the array is accessed using the value passed into the switch statement. The resulting array element is assigned to `pc` to jump to the code for the corresponding switch case.
...
...
cmp r0,#0x5
bls LAB_00048f48 // Only use switch statement if value is in range of switch cases.
b LAB_000494e0 // Go to end of function and do nothing if value is out of range.

LAB_00048f48
lsls r0,r0,#0x2 // Multiply type by 4 to get jump table array offset.
ldr r1,[DAT_00048f54] // Load jump table pointer
...
// Jump table
addr DAT_0804928c // Address to logic for cases 0 and 1.
addr DAT_0804928c
addr DAT_08049296 // Address to logic for cases 2, 3, and 5.
addr DAT_08049296
addr DAT_08049408 // Address to logic for case 4.
addr DAT_08049296
```

Small switch statements sometimes forgo a jump table and turn into a series of conditional branches instead.
...
* The center window is the __code browser__ or __listing__. This is where assembly code is displayed.
...
Right now, Ghidra is displaying the beginning of the ROM file (address 0x0), which is an entry point to the program that largely consists of boilerplate code to set up the game with the GBA engine. Let's go to a different place in the ROM that contains more interesting logic.

For this demonstration, we'll use the function at address 0x450e0. To navigate to this address, press 'g' and enter in "450e0" to make Ghidra navigate to `FUN_000450e0`. Scroll down a bit to see the whole function, which looks like this:
>![](/assets/img/reverse-engineering/ghidra-example-function.png)<br>
>FUN_000450e0 in Ghidra
...
* The __references to function__ section lists all the places in the assembly code that call the current function; the list is in the format "function-name:instruction-address". In this case, Ghidra found 396 places that call `FUN_000450e0`, one of which is `FUN_0003edf0` with a `bl` instruction at address `0x3ee08`.
...
* __Hex data__ contains the raw hexadecimal values in the ROM file that correspond to the assembly instructions. The `push { lr }` instruction at the start of `FUN_000450e0` was derived from the hex value `00 b5` in the ROM.
* The __addresses__ contain the addresses/offsets of each instruction from the start of the file. The `push { lr }` instruction at the start of `FUN_000450e0` is at address 0x450e0 in the ROM.
...
* __References to labels__ lists the places that branch to each label. Listed here is the address of each branch instruction to a given label. For example, `LAB_000450ea` is referenced by the `bne` instruction at address 0x450e4.
...
* Clicking on a function, label, or reference to a function/label will jump to the corresponding location in the ROM.
* On the toolbar at the top of the Ghidra window, there are buttons to navigate back and forward. These buttons keep track of your history when jumping to different addresses via _Go To..._ or when clicking on functions and labels.
...
### Comments
You can add comments to the assembly code by right-clicking an assembly instruction and going to _Comments_ in the context menu. There are options to position the comment in several places, including end-of-line (EOL), pre-line, post-line, and plate comments. You can also assign key bindings to these in _Edit > Tool Options > Key Bindings_.

Here is an example end-of-line comment added to address 0x450f0.
>![](/assets/img/reverse-engineering/eol-comment.png)<br>
>End-of-line comment

Liberal use of comments is highly recommended. There are no descriptive variable names in assembly, so it is easy to get lost when reading and re-reading assembly code if you don't leave notes for yourself.

### Label names
Once you figure out what a function, branch label, stack value, or data value does, you can rename it by right-clicking the label and clicking _Edit label_.

Sometimes Ghidra's disassembler will automatically label registers as parameters. This is rarely helpful because registers are reused for many variables throughout the function. An example of this is `FUN_00043af4`.
>![](/assets/img/reverse-engineering/register-labels.png)<br>
>Registers have been replaced with parameter labels

To remove these labels, right-click the function header and use _Edit Function_ to remove all the function variables from the function.

### Jump tables
Jump tables give Ghidra's disassembler some trouble, and require massaging to make them more readable.

An example occurrence of a jump table is `FUN_00048f28`.
>![](/assets/img/reverse-engineering/jump-table.png)<br>
>A jump table with unknown data

After the `mov pc,r0` and `DAT_00048f54` data value is supposed to be a jump table, but the addresses are identified as "??" instead. To fix this, right-click the unknown data and select _Data > Pointer_. You can also select multiple data values and set the data types for all of them at once. This will turn the jump table into data values so that it's more readable. If you accidentally set something to a data value that should not be, right-click and _Clear Code Bytes_ to reset it.
>![](/assets/img/reverse-engineering/jump-table-fixed.png)<br>
>Fixed jump table after making the data values into pointers

Note that the jump table uses addresses with 0x080xxxxx instead of 0x000xxxxx, as these are where instructions end up while the game is running. You can subtract 0x8000000 from the jump table address to get the address of the instruction in the ROM.

The first jump table address in this switch statement is at 0x4928c. There should be assembly instructions here, but Ghidra's disassembler doesn't automatically recognize it as code because of the way the jump table is structured.
>![](/assets/img/reverse-engineering/jump-table-code.png)<br>
>Jump table code that was not automatically disassembled

To manually disassemble this data into assembly instructions, right-click the data and _Disassemble - Thumb_.
...
### Ghidra wrap-up
By now, we've explored a handful of Ghidra's functionality, enough to get started on reading the assembly code for a game. Ghidra has a ton of other useful features for reverse engineering, so you may wish to explore on your own and discover which features strike your fancy.
...
A breakpoint can be set using the `b` (`break`) command, with the format `b <address>`. When the game is running, the ROM is loaded into the area of memory starting at `0x08000000`, so this value must be added to the function address seen in Ghidra when setting breakpoints.

As a demonstration, set a breakpoint at the beginning of the function `FUN_00044b28` using `b 0x08044b28`. This will pause game execution, so unpause the game with the `c` command.
>![](/assets/img/reverse-engineering/debugger-add-breakpoint.png)<br>
>Adding a breakpoint

With the breakpoint added, try to move around in-game. This will execute `FUN_00044b28` and hit the breakpoint, pausing the game's execution.
...
Note that the instruction displayed by the watchpoint is the instruction _after_ the instruction that accessed the watched address, meaning that the instruction that wrote to the HP value is at 0x08074C28. By looking up address 0x74C28 in Ghidra, you'll find the `strh` instruction used to set the HP value, within `FUN_00074b54`. This indicates that `FUN_00074b54` is responsible for passive HP restoration.
...
With assembly knowledge, Ghidra, and mGBA, we have all the tools to start reverse engineering a game. The last section of this tutorial will discuss strategies for reverse engineering a game.

The mGBA section of this tutorial demonstrates a couple of strategies for finding memory and code addresses using [memory search](#memory-search) and [watchpoints](#watchpoints), so check those out if you haven't already.
...
>An Oran Berry on the ground just to the top right of the player, along with the dungeon stairs in the lower left
...
The watchpoint stopped at address 0x8077CD6. Go to address 0x77CD6 in Ghidra and look at the previous instruction, and you'll see the `strh r0,[r4,#0xe]` that stored the new HP value. Note that you might need to go up to the start of the function and [delete the function's labels](#label-names) for Ghidra to show register numbers. According to the watchpoint, the current value of `r0` is 0x65 (101 in decimal). This lines up with the Oran Berry's 100 HP of healing plus the player's 1 HP. There is code later to cap the player's HP at their max HP, but the uncapped HP is workable as is.
...
The 0x64 in `r10` came from `r2`, but there are no more references to `r2` between here and the start of the function. This means that `r2` is the third parameter of `FUN_00077c44`, and the 0x64 came from the code that called `FUN_00077c44`. The next step is to find the caller of this function.

Scroll to the bottom of `FUN_00077cc44`, and you'll find the `bx` instruction at 0x77dd6 marking the end of the function. Make a breakpoint in mGBA at 0x8077dd6, then continue program execution to hit the breakpoint. Note that before hitting the breakpoint, you'll hit the watchpoint for player HP again; this is where the player's HP is capped at their max HP.

>![](/assets/img/reverse-engineering/oran-berry-end-function.png)<br>
>Hitting the breakpoint at the end of `FUN_00077cc4`

The breakpoint shows that `r0` is 0x0804838D. As the code is about to return from `FUN_00077cc4`, this is where the function will return to, and also where it was called. However, if you go to address 0x4838D in Ghidra, this section is not marked as assembly code.
>![](/assets/img/reverse-engineering/oran-berry-missing-assembly.png)<br>
>Missing assembly code at 0x4838D

Sometimes Ghidra doesn't auto-detect certain blocks of code as assembly. Select line 0x48364 (directly after the last function that is already disassembled) and press F12 to disassemble this block of code as Thumb assembly. Ghidra will disassemble this line and also follow the assembly branches and function calls to disassemble subsequent lines of assembly, including line 0x4838D.

>![](/assets/img/reverse-engineering/oran-berry-fixed-assembly.png)<br>
>0x4838D and surrounding addresses after manually disassembling it

Now the call to `FUN_00077cc44` is visible at address 0x48388. We were looking for where `r2` was assigned to 0x64, so look at the lines prior to the function call. We can see this set of instructions starting at 0x48378.
```
ldr r2,[DAT_00048394]
mov r3,#0x0
ldrsh r2,[r2,r3]
```
This loads an address into `r2` from data value `DAT_00048394`, then loads the value at the address into `r2`. Look at address 0x48394 to find that the value of `DAT_00048394` is 0x080F4FB6. This is within ROM, which is promising; ROM cannot normally be written to, so chances are that this is the source of the value 0x64.
...
Success! We've found that the Oran Berry's heal amount is stored at 0x080F4FB6, or 0xF4FB6 in the game binary (.gba) file. Technically, if you edited the game binary and changed the value at this address, you'd create a minimal ROM hack that nerfs the Oran Berry's healing. Creating ROM hacks is out of the scope of this tutorial, but this demonstrates a piece of the process for creating these hacks.
...
>![](/assets/img/reverse-engineering/eat-oran-berry.png)<br>
>The game state just before executing the instruction at 0x77CD6
...
For further confirmation we can change the `strh` instruction at `0x77ce0` so that it doesn't cap the player's HP. In Ghidra, you can see that this instruction's hex value is `e0 81`. In mGBA, go to address `0x8077ce0` in the memory viewer, and you'll find the same `e0 81` there. Change both of these bytes to `00`, which will change the instruction to a no-op (an instruction that does nothing).
...
Once you've found the address of a value in memory, it's likely that nearby values in memory are also related, possibly as part of a struct or array of data. We can use the memory viewer to look at other RAM values in the vicinity of the known value, and possibly discover more values by visual inspection or attempting to change them. This approach doesn't have a specific goal in mind; instead, it is meant to discover a bunch of values quickly, laying some groundwork to expedite the process of searching for specific values later.
